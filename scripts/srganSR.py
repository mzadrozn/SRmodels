...
#from models.SRGAN.truncattedVgg import TruncatedVGG19
...
class SrganSR(AppSR):
            ...
            for epoch in range(start, end):
                ...
                if validLoss < self.bestValidLoss:
                    ...
                    [best.unlink() for best in
                     getFiles(self.getCheckpointFolder(generator_folder=False), "best*.pth")]  # remove last best pth
            ...
            for x, y in batchLoader:
                ...
                if discriminatorOnly:
                    ...
                    with amp.autocast():
                        ...
                        adversarial_loss = 1e-3 * self.adv_loss(sr_discriminated, torch.ones_like(sr_discriminated))
    ...
    def getCheckpointFolder(self, generator_folder=True):
        if generator_folder:
            return PATHS.CHECKPOINTS / f"X{self.configs['scaleFactor']}" / (self.getGeneratorModelName())
        else:
    ...
    def getGeneratorModelName(self):
        return f"SRGAN-lr{self.configs['startLearningRate']}-generatorOnly{self.pretrain}-flip{self.configs['randomFlip']}-psize" \
               f"{self.configs['patchSize']}"
    ...
    def save(self, fileName="", pretrained='false'):
        ...
        if self.pretrain:
            ...
            saveFolder = self.getCheckpointFolder(generator_folder=False)
        mkdir(saveFolder)
